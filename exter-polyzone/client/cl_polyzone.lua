...
local function addToComboZone(zone)
  if comboZone ~= nil then
    ...
    comboZone = ComboZone:Create({ zone }, { name = "exter-polyzone" })
    comboZone:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
      if leftZones ~= nil then
        for i = 1, #leftZones do
          ...
          TriggerEvent("exter-polyzone:exit", leftZone.name, leftZone.data)
        end
      end
      if enteredZones ~= nil then
        for i = 1, #enteredZones do
          ...
          TriggerEvent("exter-polyzone:enter", enteredZone.name, enteredZone.data, enteredZone.center)
        end
      end
    end, 500)
  end
...
RegisterNetEvent("exter-polyzone:createCircleZone")
AddEventHandler("exter-polyzone:createCircleZone", function(name, ...)
  addCircleZone(name, ...)
end)
...
if GetConvar("sv_environment", "prod") == "debug" then
  RegisterCommand("exter-polyzone:debug", function(src, args)
    toggleEntityDebug(not debugEnabled)
    toggleDebug(not debugEnabled)
  end)

  RegisterCommand("exter-polyzone:insideZones", function()
